...
function FPSMeterRename(){
	...
	this.getFramerate = function(){
		...
		if(diff >= this.sampleDuration){
			...
			sampleFPS = FPSMeterRename.formatNumber(rawFPS);
			sampleFrames = 0;
...
FPSMeterRename.formatNumber = function(val){
	//format as XX.XX
...
function PerformanceBar(name, elem){
	...
	this.continueTest = function(){
		...
		if(time-testBegin > 10000){
			...
			title = "Test Results: "+FPSMeterRename.formatNumber(output)+" fps";
		}
	}
